...
exports.generateDataExport = void 0;
...
exports.generateDataExport = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (dataTypes.includes('transactions')) {
            ...
            exportData.data.transactions = transactionsSnapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { date: doc.data().date.toDate().toISOString(), createdAt: doc.data().createdAt.toDate().toISOString(), updatedAt: doc.data().updatedAt.toDate().toISOString() })));
        }
        // Export budgets
        if (dataTypes.includes('budgets')) {
            const budgetsSnapshot = await db.collection('users').doc(userId).collection('budgets').get();
            exportData.data.budgets = budgetsSnapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { createdAt: doc.data().createdAt.toDate().toISOString(), updatedAt: doc.data().updatedAt.toDate().toISOString() })));
            ...
            exportData.data.comprehensiveBudgets = comprehensiveBudgetsSnapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { createdAt: doc.data().createdAt.toDate().toISOString(), updatedAt: doc.data().updatedAt.toDate().toISOString() })));
        }
        ...
        if (dataTypes.includes('savings_goals')) {
            ...
            exportData.data.savingsGoals = savingsGoalsSnapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { deadline: doc.data().deadline.toDate().toISOString(), createdAt: doc.data().createdAt.toDate().toISOString(), updatedAt: doc.data().updatedAt.toDate().toISOString(), completedDate: doc.data().completedDate ? doc.data().completedDate.toDate().toISOString() : null })));
        }
        // Export achievements
        if (dataTypes.includes('achievements')) {
            ...
            exportData.data.achievements = achievementsSnapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { unlockedDate: doc.data().unlockedDate ? doc.data().unlockedDate.toDate().toISOString() : null })));
        }
        // Export AI insights
        if (dataTypes.includes('insights')) {
            const insightsSnapshot = await db.collection('users').doc(userId).collection('ai_insights').get();
            exportData.data.insights = insightsSnapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { generatedAt: doc.data().generatedAt.toDate().toISOString() })));
        }
        ...
        switch (format) {
            ...
            case 'csv':
                ...
                if (exportData.data.transactions) {
                    ...
                    const csvRows = transactions.map((t) => `${t.date},${t.type},${t.amount},${t.category},"${t.description}",${t.createdAt}`).join('\n');
...
//# sourceMappingURL=generateDataExport.js.map