...
const awardXP = async (data, context) => {
    if (!context.auth) {
...
async function updateLeaderboardsForXP(userId, xpGained) {
    const db = admin.firestore();
    try {
...
async function checkLevelAchievements(userId, newLevel) {
    const db = admin.firestore();
    try {
        ...
        const levelAchievements = allAchievements.filter((a) => a.category === 'Level' && a.unlockConditions.level <= newLevel);
...
//# sourceMappingURL=awardXP.js.map