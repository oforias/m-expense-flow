...
exports.analyzeSpendingPatterns = void 0;
const admin = require("firebase-admin");
const functions = require("firebase-functions");
const geminiClient_1 = require("./geminiClient");
const geminiConfig_1 = require("./geminiConfig");
exports.analyzeSpendingPatterns = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const patterns = Array.from(categoryTotals.entries()).map(([category, data]) => ({
            category,
            amount: data.amount,
        ...
        const insights = await generateInsights(patterns, totalSpent, period, userId, data.useAI);
        // Save insights to Firestore
        const insightPromises = insights.map(insight => db.collection('users')
            .doc(userId)
            .collection('ai_insights')
            .add(Object.assign(Object.assign({}, insight), { generatedAt: admin.firestore.FieldValue.serverTimestamp(), isRead: false })));
...
async function generateInsights(patterns, totalSpent, period, userId, useAI) {
    // Try AI-powered insights first if enabled
    if ((useAI || (0, geminiConfig_1.isGeminiEnabled)()) && patterns.length > 0) {
        try {
 ...
 */
async function generateAIInsights(patterns, totalSpent, period, userId) {
    const db = admin.firestore();
    ...
    const monthlyIncome = (userData === null || userData === void 0 ? void 0 : userData.weeklyIncome) ? userData.weeklyIncome * 4.33 : undefined;
    ...
    const goalLines = goals.length > 0
        ...
        : '  None';
    const prompt = `Analyze this student's ${period} spending in Ghana and return specific insights.\n\n` +
 ...
 */
function generateRuleBasedInsights(patterns, totalSpent, period) {
    const insights = [];
...
function getHighSpendingMessage(category, percentage) {
    const ghanaSpecificMessages = {
...
//# sourceMappingURL=analyzeSpendingPatterns.js.map