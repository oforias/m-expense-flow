...
exports.detectSpendingAnomalies = void 0;
const admin = require("firebase-admin");
const functions = require("firebase-functions");
exports.detectSpendingAnomalies = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const periodData = groupTransactionsByPeriod(transactionsSnapshot.docs.map(doc => (Object.assign(Object.assign({}, doc.data()), { date: doc.data().date.toDate() }))), periodDays, now);
        ...
        if (anomalies.length > 0) {
            ...
            anomalies.forEach(anomaly => {
                ...
                batch.set(docRef, Object.assign(Object.assign({}, anomaly), { detectedAt: admin.firestore.FieldValue.serverTimestamp(), period, isResolved: false }));
            });
            await batch.commit();
        }
        return {
...
function groupTransactionsByPeriod(transactions, periodDays, currentDate) {
    ...
    transactions.forEach(transaction => {
        ...
        if (periodIndex >= 6)
            return; // Only keep last 6 periods
        if (!periodData.has(periodIndex)) {
            periodData.set(periodIndex, new Map());
        }
...
function calculateCategoryStats(periodData, targetCategories) {
    ...
    for (let period = 1; period < 6; period++) { // Periods 1-5 (historical)
        ...
        allCategories.forEach(category => {
            if (targetCategories && !targetCategories.includes(category))
                return;
    ...
    categoryStats.forEach((amounts, category) => {
        if (amounts.length < 3)
            return; // Need at least 3 historical periods
...
function detectAnomalies(categoryStats) {
    const anomalies = [];
...
function generateAnomalyMessage(category, isIncrease, deviationPercentage, severity) {
...
function generateSuggestions(category, isIncrease, severity) {
    if (!isIncrease) {
...
//# sourceMappingURL=detectSpendingAnomalies.js.map