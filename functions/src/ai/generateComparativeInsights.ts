...
interface GenerateComparativeInsightsData {
  userId: string;
...
export const generateComparativeInsights = functions.https.onCall(
  async (data: GenerateComparativeInsightsData, context) => {
    ...
    try {
      ...
      // Convert peer data
      const peerData = new Map<string, PeerData>();
      peerAveragesSnapshot.docs.forEach(doc => {
        const data = doc.data() as PeerData;
        peerData.set(data.category, data);
      });

      // Generate comparative insights
      const insights = generateComparativeAnalysis(
        userCategoryTotals,
        userTotalSpent,
        peerData
      );
...
function generateComparativeAnalysis(
  userCategoryTotals: Map<string, number>,
  userTotalSpent: number,
  peerData: Map<string, PeerData>
): ComparativeInsight[] {
  ...
  userCategoryTotals.forEach((userAmount, category) => {
    ...
    if (!peerInfo || peerInfo.sampleSize < 10) return; // Need sufficient peer data
    ...
    let status: 'above_average' | 'below_average' | 'average';
    ...
    const insight: ComparativeInsight = {
      category,
      userPercentage,
      peerAverage,
      difference,
...
function generateComparisonSuggestions(
  ...
  const ghanaSpecificSuggestions: { [key: string]: { above: string[], below: string[] } } = {
    'Uber/Bolt': {
      above: [
...
async function calculatePeerAverages(db: admin.firestore.Firestore, period: string): Promise<void> {
  ...
  const categoryTotals = new Map<string, { totalAmount: number, userCount: number, percentages: number[] }>();
  ...
  categoryTotals.forEach((data, category) => {
    if (data.userCount < 5) return; // Need at least 5 users for meaningful average
