...
interface GenerateStudentRecommendationsData {
  userId: string;
...
export const generateStudentRecommendations = functions.https.onCall(
  async (data: GenerateStudentRecommendationsData, context) => {
    if (!context.auth) {
    ...
    try {
      ...
      let recommendations: StudentRecommendation[];

      if (isGeminiEnabled() && spendingData.length > 0) {
        try {
      ...
      recommendations.forEach(rec => {
        ...
        batch.set(ref, { ...rec, generatedAt: admin.firestore.FieldValue.serverTimestamp(), isRead: false });
      });
      await batch.commit();
...
async function generateGeminiRecommendations(params: {
  spendingData: Array<{ category: string; amount: number; count: number; percentage: number }>;
  totalSpent: number;
  budgets: Array<{ category: string; limit: number; spent: number }>;
  goals: Array<{ name: string; target: number; current: number; deadline?: any }>;
  ...
  const systemContext =
    `You are a financial advisor for university students in Ghana. ` +
  ...
  const prompt =
    `Student financial data for the ${params.period}:\n\n` +
