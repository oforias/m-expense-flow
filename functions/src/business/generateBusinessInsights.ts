...
interface BusinessInsight {
  type: 'revenue' | 'expense' | 'profit' | 'efficiency' | 'growth' | 'warning';
  title: string;
  message: string;
...
interface BusinessMetrics {
  ...
  categoryBreakdown: { [category: string]: { revenue: number; expenses: number } };
}

export const generateBusinessInsights = functions.https.onCall(
  async (data: BusinessInsightRequest, context) => {
    try {
      ...
      const transactions = businessTransactionsQuery.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
...
function calculateBusinessMetrics(transactions: any[]): BusinessMetrics {
  const categoryBreakdown: { [category: string]: { revenue: number; expenses: number } } = {};
...
function generateInsightsFromMetrics(
  ...
  const businessType = businessProfile?.businessType || 'general';
  ...
  switch (businessType) {
    case 'Fashion Reseller':
      if (metrics.categoryBreakdown['Inventory']?.expenses > metrics.totalRevenue * 0.6) {
        insights.push({
          type: 'warning',
  ...
  if (metrics.topExpenseCategory) {
    const topExpenseAmount = metrics.categoryBreakdown[metrics.topExpenseCategory]?.expenses || 0;
