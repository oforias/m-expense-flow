...
interface CancelTrialData {
  userId: string;
  reason?: string;
}

interface ScheduleTrialReminderData {
  userId: string;
  reminderDate: string;
  trialEndDate: string;
}

interface UpgradeFromTrialData {
  userId: string;
  planType: 'monthly' | 'semester' | 'yearly';
 ...
 */
export const startFreeTrial = functions.https.onCall(
  async (data: StartTrialData, context) => {
 ...
 */
export const cancelTrialOnUpgrade = functions.https.onCall(
  async (data: CancelTrialData, context) => {
    // Verify authentication
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
export const scheduleTrialReminder = functions.https.onCall(
  async (data: ScheduleTrialReminderData, context) => {
    // Verify authentication
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
export const upgradeFromTrial = functions.https.onCall(
  async (data: UpgradeFromTrialData, context) => {
    // Verify authentication
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
export const checkTrialExpiration = functions.pubsub
 ...
 */
async function scheduleTrialReminders(userId: string, trialEndDate: Date): Promise<void> {
  const db = admin.firestore();
