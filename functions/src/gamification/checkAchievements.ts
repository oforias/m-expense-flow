...
interface CheckAchievementsData {
  userId: string;
  context: {
    type: 'transaction' | 'budget' | 'savings' | 'streak' | 'challenge' | 'mission' | 'business';
    data?: any;
  };
}
...
export const checkAchievements = async (data: CheckAchievementsData, context: functions.https.CallableContext) => {
  if (!context.auth) {
  ...
  try {
    ...
    const allAchievements = achievementsSnapshot.data()!.achievements as Achievement[];
...
async function checkAchievementCondition(
  ...
  userData: any,
  context: CheckAchievementsData['context'],
...
async function checkTransactionAchievement(
  conditions: any,
  userData: any,
  context: CheckAchievementsData['context'],
  db: admin.firestore.Firestore,
...
async function checkBudgetAchievement(
  conditions: any,
  userData: any,
  context: CheckAchievementsData['context'],
  db: admin.firestore.Firestore,
...
async function checkSavingsAchievement(
  conditions: any,
  userData: any,
  context: CheckAchievementsData['context'],
  db: admin.firestore.Firestore,
...
function checkStreakAchievement(conditions: any, userData: any): boolean {
...
async function checkCategoryAchievement(
  conditions: any,
  userData: any,
  context: CheckAchievementsData['context'],
  db: admin.firestore.Firestore,
...
function checkSpecialAchievement(
  ...
  if (conditions.type === 'business_mode') {
    return context.type === 'business' && context.data?.enabled === true;
  }

  return false;
}

async function checkMetaAchievement(
  conditions: any,
  userData: any,
  context: CheckAchievementsData['context'],
  db: admin.firestore.Firestore,
